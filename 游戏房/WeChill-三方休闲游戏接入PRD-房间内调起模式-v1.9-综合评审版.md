...
> 版本：v1.9 综合评审版  
> 日期：2026-05-21  
> 整合来源：v1.8 综合评审版 + v1.7 补充复核 + 入口场景流程图汇总 + 用户补充业务逻辑  
> 整合规则：以 v1.8 为底座复核 v1.7 与入口场景流程图，修正 v1.8 中 Private room 首次交互、房主退出裁决、表格结构和入口互斥表述；保留 v1.8 已吸收的 v1.7 Bytesun 对接、门票预扣、入口决策树等内容；竞品分析单独置于文末，仅作为体验参考，不改变 WeChill 的房间生成和交互主路径。
...
- 修正 Private room：游戏选择发生在 Discover 当前页半屏内；选中游戏后才进入自己的房间并直接打开 H5，首次不展示房间页和设置页。
- 修正房主退出裁决：V1 不做管理员接管；房主主动退出时关闭游戏房并按状态退款/异常结算；房主异常断线时 waiting / ready 关闭并退款，playing / settling 可由 Bytesun 侧继续到本局结束，结束后关闭 session。
- 合入入口场景流程图中的退出场景：区分普通用户退出游戏、用户退出房间、房主关闭游戏但不退出房间、普通用户被踢出游戏座位 / 房间。
- 竞品 TopTop / Hawa 只作为体验优化参考：可采纳退出分层、加载进度、结果倒计时、房间卡片游戏标识、最小化召回文案；不采纳浮动游戏卡片、强制游戏详情页、自动匹配作为 V1 主路径。
...
| Discover 普通入口 | 可从 Discover 选择游戏 | 明确先判断用户是否在房间 | 已合入 | 保留 v1.7：在任何房间最小化时，先退出当前房间，再作为普通用户匹配游戏房 |
| Private room 首次交互 | 当前 Discover 页半屏选游戏，选后直开 H5 | 进入自己房间后展示游戏选择面板 | v1.8 仍在 6.12.2 残留“进房后选游戏” | 采用最新原型：先在 Discover 半屏选游戏，选后进入自己的房间并直接 H5，首次不展示房间页/设置页 |
| 房主退出处理 | playing / settling 可 system 托管 | 房主退出=游戏房关闭 | v1.8 顶部写关闭，正文又允许 playing 继续 | 分场景裁决：主动退出=关闭游戏房并处理退款/异常结算；异常断线=短时保留，超时后 waiting / ready 关闭，playing / settling 由 Bytesun 侧完成本局后关闭 session |
...
| 对比项 | v1.8 | v1.9 |
|---|---|---|
| 完整度 | 已合入 v1.7 绝大多数新增场景，是完整 PRD 主体 | 完整度与 v1.8 基本一致，并补齐 v1.8 的交互冲突说明 |
| 可评审性 | 内容多但部分表格和裁决口径有歧义 | 表格结构修正，关键裁决集中说明，更适合评审 |
| Private room | 页面截图正确，但 6.12.2 流程仍像“进房后选游戏” | 与最新原型一致：Discover 半屏选游戏，选后进入自己的房间并直接 H5 |
| 房主退出 | 顶部裁决与正文不完全一致 | 统一为“不接管；主动退出关闭；异常断线时已开局可由 Bytesun 完成本局后关闭” |
| 风险 | 评审时容易追问“主动退出和断线到底怎么处理”“Private room 到底哪里选游戏” | 两个高风险追问已前置裁决 |
...
| Hawa 自动匹配倒计时 | 否，V1 不做 | V1 保持 Game Rooms 列表进入；可作为 V2 Quick Match 能力评估 |
...
| V1 自动匹配 / Quick Match | 需要匹配池、倒计时、兜底和风控策略 | V2 结合数据评估 |
...
- 不存在管理员接管游戏的概念。房主主动退出 = 游戏房关闭；异常断线按分层规则处理。
...
| 默认值 | Private room 快捷入口使用后台默认配置直接进入 H5；房内 Games 面板仍展示设置页 |
...
flowchart TD
    ...
    H -->|"无"| I
    I --> K["用户=当前房间房主"]
    K --> L["预扣门票（frozen，0 门票则生成 0 金额记录或跳过冻结）"]
    L --> M{"预扣成功"}
    M -->|"游戏币不足"| N["弹窗 → 兑换页"]
    N --> O["兑换成功 → 返回"]
    O --> L
    M -->|"成功"| P["创建 RoomGameSession"]
    P --> Q{"创建成功"}
    Q -->|"失败"| R["退还冻结门票 → 提示"]
    Q -->|"成功"| S["直接加载 H5（getConfig 中 role=2 房主）"]
    S --> T{"gameLoaded（30s 超时）"}
    T -->|"失败/超时"| U["退还门票 → Retry / Close"]
    T -->|"成功"| V["等待 type=13（游戏开始）"]
    V --> W{"收到 type=13"}
    W -->|"收到"| X["确认扣费 → waiting/playing 状态"]
    W -->|"60s 超时未收到"| Y["退还门票"]
```
...
- 选中游戏后进入自己的房间并直接打开 H5，首次不展示语聊房主界面和设置页。
...
| 入口 | 用户身份 | 前置条件 | 流程 |
|---|---|---|---|
| Private room | 房主 | 可不在房间；若在别人房间需先退出；若在自己房间且已有游戏则拦截 | Discover 半屏选游戏 → 进入自己房间 → 预扣 → 创建 session → 直接 H5 |
...
| 已开局优先不中断 | 异常断线场景下，playing / settling 状态优先保障本局完成和结算稳定；主动退出按退出规则处理 |
| 钱包订单必须幂等 | 扣费、退款、结算、补偿均需订单关联和幂等处理 |
| 不做接管 | 不存在管理员接管；房主主动退出即关闭游戏房，异常断线则按分层规则处理 |

### 14.1 房间相关

| 场景 | 处理 |
|---|---|
| 当前房间房主主动退出房间 | 关闭游戏房；waiting / ready 关闭 session 并退款；playing 按异常结算或退款策略处理；settling 等结算完成后退出 |
...
> **v1.9 关键裁决：** 采用 v1.7 的"不做管理员接管"方案，同时区分主动退出与异常断线。主动退出视为关闭游戏房；异常断线在 playing / settling 中允许 Bytesun 侧完成本局，结束后关闭 session。

#### 14.2.1 核心原则

**房主主动退出 = 游戏房关闭。** 因为游戏房就是房主自己的房间，房主主动离开时，游戏房不再继续开放新局。
...
异常断线不是主动退出。为避免误杀本局和结算，断线按 14.2.2 分层处理。
...
| 房主退出走游戏房关闭逻辑 | 当前房间房主主动退出房间时，按 14.2 主动退出规则关闭游戏房并处理退款 / 异常结算 |
...
| 当前房间房主 | 按 14.2 处理：主动退出关闭游戏房；settling 需等待结算完成 |
...
flowchart TD
    ...
    B -->|"playing"| D["确认弹窗：游戏进行中，强制关闭可能触发异常结算或退款"]
    B -->|"settling"| E["不允许关闭：结算中请稍候"]
    C --> F{"确认关闭？"}
    D --> F
    F -->|"取消"| G["继续当前游戏"]
    F -->|"确认"| H["关闭 RoomGameSession"]
    H --> I{"状态"}
    I -->|"waiting / ready"| J["退还已预扣门票"]
    I -->|"playing"| K["按异常结算 / 全额退款策略处理"]
    J --> L["通知所有普通用户"]
    K --> L
    L --> M["清理游戏入口和浮窗"]
    M --> N["房间恢复普通语聊房状态"]
...
| playing | 二次确认后允许强制关闭，按异常结算或退款策略处理 |
...
```text
关闭后所有用户将退出当前游戏，房间会恢复为普通语聊房，是否继续？
```
...
| 第 3 周 | 设置页、上座/下座、观战、声音控制、门票预扣、房间系统消息 | 座位状态、type=13/14/15/16/18、ticket_order 预扣/确认/退还 | 房间游戏监控、操作日志 | 能进入 waiting / playing，门票预扣流程完整 |
...
- 最小化入口状态能实时更新（推送/轮询）。
- 房主主动退出 = 游戏房关闭并按状态退款 / 异常结算；房主异常断线时 waiting/ready 退款，playing 本局继续后关闭。
...
| 房主退出 | V1 不做管理员接管；主动退出关闭游戏房，异常断线按分层规则处理 |
| 横屏游戏 | V1 不接入，V2 支持 |
...
> 本章节基于《WeChill-角色入口场景流程图汇总》中的 TopTop / Hawa 竞品路径整理，单独放在文档最后。竞品仅作为样式与体验参考，不改变 WeChill V1 的核心方案：**语聊房内调起、Discover 半屏选游、Private room 进入自己房间、全屏 H5 WebView、RoomGameSession 单游戏会话**。
...
| 首页入口 | 游戏网格 → 游戏详情页 → Start / Create / Search / Recommended Rooms | Discover 游戏卡片 → Matching 倒计时自动匹配 | V1 不新增游戏详情页和自动匹配；保留推荐游戏 / Game Rooms / Private room |
| Private room | 首页 Private room + 创建私密房间开局 | 自己房间 Mini Game 面板开局 | WeChill 先在 Discover 半屏选游戏，再进入自己房间直接 H5 |
...
| Discover 自动匹配 | 缩短进入路径 | V1 不做 Quick Match；V2 可评估匹配池和倒计时取消 |
...
| C-09 | Quick Match 自动匹配 | Hawa | V1 不采纳 | 需要匹配池、倒计时取消、失败兜底和风控策略 |
