...
> 版本：v1.8 综合评审版  
> 日期：2026-05-20  
> 整合来源：v1.6 可评审版 + v1.7 补充修订版 + 用户补充业务逻辑  
> 整合规则：以 v1.6 为文档底座，按对比分析报告取值表逐章节合并 v1.7 修订内容；统一为房主+普通用户二角色体系；门票采用预扣+确认方案；补充 Bytesun 对接 11 个新章节。
...
- 以 v1.6 的完整文档结构为底座，保留章节编号体系。
- 按对比分析报告的取值表逐章节替换/叠加 v1.7 修订内容。
...
|| 冲突项 | v1.6 | v1.7 | v1.8 裁决 ||
|---|---|---|---|---|
|| 角色模型 | 房主/普通用户/观战独立角色 | 房主+普通用户二角色体系 | 采用 v1.7：房主+普通用户，观战为普通用户的状态 |
|| 房主退出处理 | system 托管 | 房主退出=游戏房关闭 | 采用 v1.7：不存在接管，游戏房关闭 |
|| 门票扣费时序 | type=13 时扣费 | 预扣（frozen）+确认（success） | 采用 v1.7：预扣+确认方案 |
|| Private room | 交互细节完整 | 定义清晰但缺首次交互细节 | v1.7 定义 + v1.6 交互细节（首次不展示房间页） |
|| 入口路径 | 6.0 总体链路（概要） | 6.12 完整决策树 | 保留 v1.6 的 6.0 总览 + v1.7 的 6.12 详细子流程 |
|| Bytesun 对接 | 基础接口清单 | 14.28-14.38 共 11 个新章节 | 全部采用 v1.7 |
|| 错误码 | 20 个自定义码 | 补充 Bytesun 原生码 1001-1023 | 合并为两层映射表 |
...
|| 角色 | 判定方式 | 说明 ||
|---|---|---|
|| 当前房间房主 | `room.owner_user_id == current_user_id` | 在自己房间的用户就是房主；可开启游戏、关闭游戏、调整局参数、踢出游戏座位 |
...
|| 门票预扣 | 点击游戏后，在直接打开 H5 前完成门票预扣（frozen）；H5 加载失败或超时自动退还（refunded） |
...
flowchart TD
  ...
  M --> N{"预扣成功"}
  N -->|"失败"| O["提示预扣失败，引导兑换"]
  N -->|"成功"| P["创建 RoomGameSession，记录 source=private_room_entry"]
  P --> Q["直接加载对应 Bytesun H5"]
  Q --> R{"gameLoaded（30s 超时）"}
  R -->|"失败/超时"| S["退还门票 → Retry / Close"]
  R -->|"成功"| T["等待 type=13 → 确认扣费（success）"]
  T --> U["用户在 H5 内开始游戏"]
  U --> V["最小化时返回自己的房间主界面"]
```
...
|| 门票预扣 | 点击游戏后在直接打开 H5 前完成门票预扣（frozen）；H5 加载失败或超时自动退还 |
...
|| 房主在其最小化期间离开 | 按房主退出=游戏房关闭规则处理（14.2） |
...
flowchart TD
    ...
    C -->|"不在房间"| E["以普通用户身份进入目标 Game Room"]
    C -->|"在自己的房间（最小化）"| F["提示：需退出当前房间"]
    C -->|"在别人的房间（最小化）"| G["提示：需退出当前房间"]
    F --> H{"确认退出？"}
    G --> H
    H -->|"确认"| I["退出房间（释放麦位/游戏座位）"]
    H -->|"取消"| J["留在当前状态"]
    I --> E
    
    D -->|"不在房间"| K["进入自己的房间 → 开启游戏"]
    D -->|"在自己的房间（最小化）"| L["恢复自己的房间 → 开启游戏"]
...
```mermaid
flowchart TD
    A["用户点击 Private room"] --> B["进入自己的房间（用户=房主）"]
    B --> C["展示游戏选择面板"]
    C --> D["选择一个游戏"]
    D --> E{"当前自己房间是否已有游戏"}
    E -->|"有"| F["提示：当前已有游戏进行中"]
    ...
    I --> J["兑换成功 → 返回"]
    J --> G
    H -->|"成功"| K["创建 RoomGameSession"]
    K --> L{"创建成功"}
    L -->|"失败"| M["退还冻结门票 → 提示"]
    L -->|"成功"| N["加载 H5（getConfig 中 role=2 房主）"]
    N --> O{"gameLoaded（30s 超时）"}
    O -->|"失败/超时"| P["退还门票 → Retry / Close"]
    O -->|"成功"| Q["等待 type=13（游戏开始）"]
    Q --> R{"收到 type=13"}
    R -->|"收到"| S["确认扣费 → waiting 状态"]
    R -->|"60s 超时未收到"| T["退还门票"]
```

**Private room 首次交互细节：**

- 首次不展示语聊房主界面和设置页，直接打开 H5。
...
|| 入口 | 用户身份 | 前置条件 | 流程 ||
|---|---|---|---|
|| Private room | 房主 | 不在别人房间（否则先退出） | 进入自己房间 → 选游戏 → 设置 → Start |
...
|| game_controller_user_id | 当前游戏控制人，V1 等于当前房间房主；房主离开后标记 system（仅备用，不存在接管） |
|| controller_source | host / system，V1 只用 host |
...
|| return_context | room / discover_private / discover_recommend / more_games，用于最小化和结束后返回对应页面 |
|| ticket_order_status | frozen / success / refunded / compensating，当前局门票订单状态 |
|| active_game_session_id | 用户级：当前用户活跃的游戏 session ID，用于互斥判断 |
|| minimize_source_page | room / discover_games / discover_private，最小化后返回的页面 |
...
|| discover_private | 从 Private room 快捷入口 | 回到 Discover Games 首页 |
...
- 仅 Private room 来源最小化后返回 Discover Games 首页（因其不进入语聊房主界面）。
...
|| 房主退出 = 游戏房关闭 | 不存在接管，游戏房就是房主的房间 |

### 14.1 房间相关

|| 场景 | 处理 ||
|---|---|
|| 当前房间房主离开房间 | waiting / ready 关闭 session 并退款；playing / settling 本局继续（由三方游戏托管），结束后关闭 |
...
> **v1.8 关键变更：** 采用 v1.7 的"房主退出 = 游戏房关闭"方案，移除管理员接管和 system 托管逻辑。

#### 14.2.1 核心原则

**房主退出 = 游戏房关闭。** 因为游戏房就是房主自己的房间，房主离开了，游戏房就不存在了。

不存在"接管"概念：
- 不存在管理员接管 game_controller。
- game_controller 不可转移。
- 不存在"无主游戏房"。
...
flowchart TD
    ...
    B -->|"无"| C["正常退出房间"]
    B -->|"有"| D{"游戏状态"}
    D -->|"waiting/ready"| E["弹窗：退出将关闭游戏房，所有玩家将被移出"]
    D -->|"playing"| F["弹窗：游戏进行中，退出将强制关闭游戏"]
    D -->|"settling"| G["提示：结算中，请稍候再退出"]
    ...
    I --> K["waiting/ready → 所有门票退款"]
    I --> L["playing → 按异常结算或全额退款"]
    K --> M["通知所有普通用户"]
    L --> M
    M --> N["普通用户自动退出房间 → 回到 Discover"]
    N --> O["房主退出房间"]
```
...
|| 旧版设计 | 新版设计 | 原因 ||
|---|---|---|
|| 房主离开 → 管理员接管 | 房主离开 → 游戏房关闭 | 游戏房就是房主的房间，房主离开=房间关闭 |
|| game_controller 可转移 | game_controller = 房主，不可转移 | 简化角色模型 |
|| system 托管模式 | 不存在 | 没有"无主游戏房"的概念 |

**注意：** 如果 playing 中房主断线，本局可以继续（由 Bytesun 游戏侧继续运行），但本局结束后不能再开新局。
...
|| 房主在最小化期间离开房间 | waiting / ready 关闭或退款；playing / settling 本局继续后关闭，普通用户按房主退出规则处理 |
...
|| 房主退出走游戏房关闭逻辑 | 当前房间房主主动退出房间时，按 14.2 房主退出=游戏房关闭处理 |
...
> **v1.8 关键变更：** 明确被踢用户在 settling 中的结算资格。
...
**核心原则：订单一旦进入 settling/success 状态，不因房间操作回滚。被踢用户的结算结果通过系统消息或钱包流水可查。**
...
|| 用户选择游戏 | 关闭半屏，预扣门票，进入用户自己的房间，创建 RoomGameSession，并直接进入对应 H5 |
|| 首次不展示房间页 | 直接打开 H5，不先展示语聊房主界面和设置页 |
...
|| return_context=discover_private | 最小化后返回 Discover Games 首页 |
...
|| 在自己房间（有游戏） | 点击 Discover 某个 Game Room | 提示退出 → 确认后退出自己房间+游戏 → 进入目标 |
|| 在自己房间（有游戏） | 再次在 Games 面板选另一个游戏 | 拦截：当前已有游戏进行中 |
|| 在别人房间（有游戏） | 点击 Private room | 提示退出 → 确认后退出 → 进自己房间 |
|| 在别人房间（有游戏） | 点击另一个 Game Room | 提示退出 → 确认后退出 → 进入新 Game Room |
|| 在别人房间（无游戏） | 同上 | 同上（必须退出当前房间） |
...
|| 场景 | 文案 ||
|---|---|
|| 在自己房间有游戏，要去别人游戏房 | 你正在游戏中，进入其他房间将关闭当前游戏，是否继续？ |
|| 在别人房间有游戏，要回自己房间 | 你正在游戏中，退出将同步退出当前游戏，是否继续？ |
|| 在别人房间有游戏，要去另一个游戏房 | 你正在游戏中，进入其他房间将退出当前游戏，是否继续？ |
|| 在别人房间无游戏，要去另一个房间 | 退出当前房间？ |
...
|| 第 3 周 | 设置页、上座/下座、观战、声音控制、门票预扣 | 座位状态、type=13/14/15/16/18、ticket_order 预扣/确认/退还 | 房间游戏监控、操作日志 | 能进入 waiting / playing，门票预扣流程完整 |
...
> 综合合并 v1.6 和 v1.7 的独特验收条目，并纳入门票预扣、互斥规则、房主退出=游戏房关闭等 v1.8 新规则。
...
- 最小化入口状态能实时更新（推送/轮询）。
- 房主退出 = 游戏房关闭，waiting/ready 退款，playing 本局继续后关闭。
- 房主断线 > 120s 后按分层处理规则关闭或允许本局继续后关闭。
- playing 中不可通过 UI 踢出玩家座位。
- 被踢出房间的用户在 settling 中仍有结算资格。
...
这样可以在不改造房间模型、推荐流、麦位和基础权限的情况下，最快验证游戏对房间停留、活跃和付费转化的提升。V1 重点跑通 Discover Games、国家筛选、Private room 快捷建房开游、房间内入口、设置页、H5 容器、座位同步、门票预扣与确认、游戏币结算、金币兑换游戏币、最小化恢复和后台监控。待核心数据稳定后，再考虑 Game Rooms 深度运营、随机匹配、游戏房类型、任务和锦标赛等扩展能力。
...
|| v1.8 新增 | ticket_order 完整数据模型、两层错误码映射表、入口×身份矩阵、互斥判断流程图、房主断线分层处理流程图、房主主动退出流程图 | 补充缺失内容 |

---

*v1.8 综合评审版完毕（2026-05-20）*
