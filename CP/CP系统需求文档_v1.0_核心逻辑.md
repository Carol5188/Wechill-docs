...
| 幸运礼物亲密度 | 10 金币 = 1 亲密度 | 按实际扣减金币计算 |
| 组建CP礼物亲密度 | 礼物金币 × CP礼物亲密度倍率 | **仅在邀请被接受（CP关系正式建立）时触发一次**；倍率取发起邀请时冻结的CP礼物配置，不校验CP礼物等级 |
| 组建CP礼物财富值 | 1 金币 = 1 财富值 | **发起者获得**；普通送礼按系统规则给送礼方增加财富值 |
| CP礼物亲密度 | 礼物实际金币价值 × 后台已发布亲密度倍率 | 倍率允许两位小数，只能填写大于0的数值，不设上限；经营类CP礼物需达到对应CP等级才可赠送 |
> 原型图中"赠送该CP礼物可加1-5倍增长亲密度"为展示文案，实际亲密度增加值按“礼物实际金币价值 × 礼物后台已发布亲密度倍率”计算。
...
| 财富值 | 是 | 否 | 关系经营展示数据 | CP关系内送礼方赠送任何有效付费礼物时增加；组建CP礼物在邀请被接受时给发起者增加；后续普通礼物、幸运礼物、CP礼物按实际扣减金币给送礼方增加财富值 |
...
| 邀请有效期 | 固定6小时，以系统后台东三区时间计算 | CP1.0不支持修改邀请有效期 |
...
| 幸运礼物 | 亲密度 = 实际扣减金币 ÷ 10 | 已绑定CP | 选择礼物 → 点击赠送 → 扣减金币 → 按10金币=1亲密度实时增长 → 进度条动画填充 |
| 组建CP礼物 | 亲密度 = 礼物金币 × 邀请快照中的CP礼物亲密度倍率 | **仅在邀请被接受时触发一次**；发起邀请时跳过CP礼物等级校验 | 被邀请者点击「接受」→ 建立CP关系 → 亲密度一次性计入 |
| CP专属礼物（普通） | 亲密度 = 礼物实际金币价值 × 后台已发布亲密度倍率 | CP等级 ≥ 礼物解锁等级 | 点击CP礼物 → 扣减金币 → 亲密度增长 → 展示送礼特效 |
...
| 奖励维度 | 奖励均自动到账；关系维度奖励按“用户 + CP关系 + 等级 + 奖励项”去重，用户维度奖励按“用户 + 等级 + 奖励项”去重 |
...
| 升级时机 | 亲密度写入后实时检测 | - |
| 配置版本 | 后台修改等级配置时生成新配置版本；1.0由列表页“一键发布配置”直接发布 | 新建立CP关系使用最新完整等级表；已建立关系发布时保持当前等级，后续有效送礼后按最新等级表检测，计算结果仅允许维持或升级；历史已发放奖励和历史榜单快照不追溯 |
...
| 等级下限 | 已建立关系的当前等级为最低等级，配置发布或阈值变化后只允许维持或升级 |
...
| 被邀请者未开启「自动拒绝CP邀请」 | 轻提示「对方暂不接受CP邀请」；不创建邀请、不扣款、不生成退款记录 |
| 发起者不存在其他有效主动邀请 | 轻提示「你已有邀请待处理」；若目标用户已向当前用户发出待处理邀请，则进入双向邀请自动接受流程 |
| 创建新邀请时金币余额充足 | 仅未命中双向邀请时校验；余额不足提示「金币余额不足，请充值」并拉起半屏充值页面 |
...
| 已拒绝 | 已拒绝 | 被邀请者点击「狠心拒绝」/ 超过有效期未处理 / 被邀请者先与其他用户建立CP关系 | 是 | 否 | 轻提示「已拒绝」 |
| 已取消 | 已取消 | 邀请发起者先与其他用户建立CP关系，系统取消其已发出的待处理邀请 | 是 | 否 | 轻提示「邀请已取消」 |

**状态命名规范（强制）**：

- 邀请超时统一归入 **已拒绝**
- 被邀请者先与其他用户建立CP关系时，该邀请统一归入 **已拒绝**
- 邀请发起者先与其他用户建立CP关系时，其已发出的待处理邀请统一归入 **已取消**
...
#### 异常与测试关注点
- 覆盖开关切换即时生效、开启后新邀请在创建前被拦截、关闭后恢复新邀请创建能力
...
| 配置项 | 默认值 | 生效策略 | 说明 |
|---|---|---|---|
| 邀请有效期 | 固定6小时 | CP1.0固定口径 | 不支持后台修改 |
| CP礼物亲密度倍率 | 后台配置 | 新配置即时生效 | 经营类CP礼物在支付确认时读取最新倍率；组建CP礼物在邀请创建时冻结倍率快照 |
| 扩展规则 | CP1.0不启用 | 相关规划见《CP2.0需求文档》 | - |
| 等级配置 | 12级阈值表 | 新配置生成新版本；新关系直接使用，已建立关系在后续有效送礼后仅检测维持或升级 | 配置发布不调整既有等级，不回溯历史奖励 |
...
| 幸运礼物亲密度 | 10 金币 = 1 亲密度 | 按实际扣减金币计算 |
| CP礼物亲密度 | 礼物实际金币价值 × 后台配置倍率 | 以礼物后台已发布配置为准；组建CP礼物使用邀请创建时冻结的倍率快照 |

#### 配置管理
| 配置项 | 配置方式 |
|---|---|
| 金币汇率 | 后台全局配置 |
| CP礼物亲密度倍率 | 礼物级别配置，按礼物单独配置亲密度倍率 |
...
### 二、CP邀请状态流转图
#### 状态枚举
CP邀请统一使用以下4种状态：待处理、已接受、已拒绝、已取消。

#### 状态流转
```mermaid
stateDiagram-v2
    [*] --> 待处理: 发起邀请成功（扣金币、创建记录、有效期6小时）
    待处理 --> 已接受: 被邀请者点击接受
    待处理 --> 已拒绝: 被邀请者点击拒绝（手动）
    待处理 --> 已拒绝: 超过6小时未处理（系统）
    待处理 --> 已拒绝: 被邀请者先与其他用户建立CP关系
    待处理 --> 已取消: 邀请发起者先与其他用户建立CP关系
    
    已接受 --> [*]: 创建CP关系+初始化亲密度+按角色清理其他邀请
    已拒绝 --> [*]: 退回发起者金币
    已取消 --> [*]: 退回发起者金币
...
| 状态 | 进入条件 | 退出条件 | 退款口径 |
|---|---|---|---|
| 待处理 | 发起邀请成功，扣金币，设置6小时有效期 | 被接受/拒绝/取消/超时 | 不退款 |
| 已接受 | 被邀请者点击接受，校验通过 | 终态，不可逆 | 不退款（金币用于组建CP礼物） |
| 已拒绝 | 手动拒绝、超过6小时未处理或系统按拒绝规则清理 | 终态，不可逆 | 全额退回发起者 |
| 已取消 | 邀请发起者建立其他CP关系后，系统取消其已发出的待处理邀请 | 终态，不可逆 | 全额退回发起者 |
...
2. 更新邀请状态为已接受
3. 发起者增加财富值（1金币=1财富值）
4. 双方增加亲密度（按邀请快照中的组建CP礼物金币价值 × 后台配置倍率写入）
...
### 六、亲密度增加业务总流程
#### 模块定位
本模块用于统一 CP 关系内所有“亲密度增加”来源的总流程。CP1.0仅保留送礼型增长：接受邀请初始化、普通礼物、幸运礼物、CP礼物。

#### 适用入口
| 入口类型 | 是否增加总亲密度 | 是否增加本周期贡献值 | 是否写入亲密度记录 | 说明 |
|---|---|---|---|---|
| 接受 CP 邀请后初始化 | 是 | 是 | 是 | 按邀请快照中的“组建CP礼物金币价值 × 后台配置倍率”写入，作为关系建立后的初始亲密度 |
| 普通礼物送礼 | 是 | 是 | 是 | 已绑定关系内的常规付费增长入口；1金币=1亲密度 |
| 幸运礼物送礼 | 是 | 是 | 是 | 按实际扣减金币计算；10金币=1亲密度 |
| CP 礼物送礼 | 是 | 是 | 是 | 亲密度=礼物实际金币价值×后台已发布亲密度倍率 |
...
读取组建CP礼物快照]
    C -->|普通/幸运/CP礼物| C2[读取礼物类型
实际金币价值与CP礼物倍率]

    C1 --> D1[计算初始化亲密度增加值
组建礼物金币价值×邀请倍率快照]
    C2 --> D2[按礼物类型计算增加值
普通1比1 幸运10比1 CP金币价值乘倍率]
...
| 礼物增长规则 | 普通礼物、幸运礼物、CP礼物都属于已绑定关系内的礼物型增长，分别按1金币=1亲密度、10金币=1亲密度、礼物金币价值×后台倍率计算 | 礼物结果成立后，亲密度、财富值和榜单贡献按实际规则正常结算 |
...
##### 分支流程
- 校验失败：禁止发布，草稿保留供运营修正。
- 发布成功：新建立关系使用最新完整等级表；已建立关系保持发布前等级，后续有效送礼后按最新等级表检测，结果仅允许维持或升级。礼物与建立关系配置仅作用于发布后的新业务行为。
...
- 等级配置发布不调整已建立关系的当前等级，后续仅允许维持或升级。
...
##### 触发条件
- 用户选择了组建 CP 礼物。
- 双方满足邀请条件；仅创建新邀请时要求金币余额充足，命中双向邀请时不校验或扣减后发起方金币。
...
flowchart TD
    ...
    B --> C{双方是否满足邀请条件}
    C -->|否| D[提示不可邀请原因]
    C -->|是| C1{对方是否已向自己发出待处理邀请}
    C1 -->|是| C2[接受对方已有邀请 不创建第二条邀请 不二次扣款]
    C2 --> K[创建CP关系]
    C1 -->|否| E{金币余额是否充足}
    E -->|否| F[提示余额不足并拉起充值]
    E -->|是| G[扣减发起者金币]
...
##### 分支流程
- 双向邀请：后发起方直接接受对方已有邀请，使用原邀请礼物及配置快照完成关系建立，不创建第二条邀请、不校验余额、不二次扣款。
- 接受：建立 CP 关系，一次性计入亲密度和财富值，并发放结成奖励和特效。
- 拒绝 / 过期 / 取消：退回组建礼物金币，刷新邀请状态并通知发起者；被邀请者先建立其他CP关系时记为已拒绝，邀请发起者先建立其他CP关系时记为已取消。
...
- 组建 CP 礼物亲密度按“邀请快照中的礼物金币价值 × 邀请快照中的后台配置倍率”写入；财富值按“礼物实际支付金币 × 1”给发起者增加。
- 接受后刷新主页、资料页、房间和私聊关系状态。

##### 幂等/补偿
- 邀请终态及并发处理规则遵循“8.1 CP邀请闭环流程”的“幂等/补偿”；本链路的退币补偿、奖励补发必须复用原邀请记录和原礼物快照，不得新建第二条主记录。

#### 9.3 普通礼物、幸运礼物与CP礼物送礼增长流程
##### 流程目标
定义普通礼物、幸运礼物、CP礼物在已绑定关系下的送礼增长口径，确保扣费、亲密度、榜单贡献、等级校验和超上限提示遵循同一顺序。
...
```mermaid
flowchart TD
    A[用户选择普通礼物、幸运礼物或CP礼物] --> B{CP关系是否正常}
    B -->|否| C[提示当前无有效CP关系]
    B -->|是| D{礼物是否满足等级门槛}
    D -->|否| E[提示未达到解锁等级]
    D -->|是| F{金币余额是否充足}
    ...
    H --> I[生成送礼流水]
    I --> J{礼物类型}
    J -->|普通礼物| K[亲密度增量=实际扣减金币×1]
    J -->|幸运礼物| K1[亲密度增量=实际金币数量÷10]
    J -->|CP礼物| L[亲密度增量=礼物金币价值×后台配置倍率]
    K --> N[校验每日亲密度规则]
    K1 --> N
    L --> N
    N --> S[更新总亲密度和本周期榜单贡献]
...
##### 分支流程
- 普通礼物：亲密度按“金币 × 1”计算。
- 幸运礼物：亲密度按“实际扣减金币 ÷ 10”计算。
- CP礼物：亲密度按“礼物实际金币价值 × 后台已发布亲密度倍率”计算。
...
##### 后台处理
- 亲密度写入必须同时记录来源：普通礼物、幸运礼物、CP礼物，方便记录页和客服追溯。
...
| 角色 | 职责 |
|---|---|
| 用户 | 查看礼物记录、奖励到账结果或关闭结果弹窗 |
| 系统后台 | 写送礼记录、生成奖励记录、自动发放并处理过期 |
| 客服 / 运营 | 追溯和补发异常奖励 |

##### 触发条件
- 送礼成功或奖励命中。
- 所有奖励均按规则自动发放，不存在用户手动领取后才到账的奖励类型。
...
- 无奖励：记录页只展示送礼明细。
- 奖励统一自动到账，并更新为已发放。
- CP物料奖励：若获得更高等级的同类型 CP 物料，系统回收背包内旧的低等级同类型 CP 物料，并发放新的高等级 CP 物料。
- 已佩戴替换：若用户当前已佩戴低等级同类型 CP 物料，系统自动更换为新获得的高等级 CP 物料。

##### CP物料奖励替换业务流程图
```mermaid
flowchart TD
    A[CP等级升级命中奖励] --> B{奖励是否为CP物料}
    B -->|否| C[按普通奖励发放]
    B -->|是| D{背包内是否已有同类型低等级CP物料}
    D -->|否| E[直接发放新CP物料]
    D -->|是| F[回收旧低等级同类型CP物料]
    F --> G[发放新高等级同类型CP物料]
    E --> H{当前是否佩戴同类型低等级CP物料}
    G --> H
    H -->|否| I[仅更新背包物料]
    H -->|是| J[自动更换为新高等级CP物料]
    I --> K[写入奖励记录和物料变更记录]
...
| 奖励记录状态 | 进入条件 | 退出条件 |
|---|---|---|
| 自动发放中 | 奖励规则命中、生成唯一奖励记录并启动自动发放 | 发放成功 / 发放失败 |
| 发放失败待重试 | 自动发放失败 | 重试成功 / 人工终结 |
| 已发放 | 自动发放或重试成功 | 终态 |
| 人工终结 | 重试仍失败且具备人工终结权限 | 终态，保留失败原因、操作人和操作时间 |

##### 异常分支
- 奖励弹窗只展示自动发放结果；“收下”按钮仅关闭弹窗，不触发发放、领取或补发。
...
- 关系维度奖励按“发放对象用户ID + CP关系ID + 等级 + 奖励项ID”生成唯一奖励记录；用户维度奖励按“发放对象用户ID + 等级 + 奖励项ID”生成唯一奖励记录。
- 带奖励的 CP 礼物，需要区分邀请者视角和被邀请者视角展示不同文案，但奖励结果都以最终记录为准。
- CP物料奖励按“物料类型 + CP等级”判断替换关系；同类型更高等级物料获得后，低等级同类型物料不得继续保留在背包中。
- 用户已佩戴低等级同类型 CP 物料时，替换发放成功后必须自动切换为新高等级物料；头像框、头饰框、气泡框、主页背景、连线等 CP 相关物料同理。
- 奖励说明弹层、中奖弹层、记录页和客服后台都必须回到同一份奖励记录。

##### 幂等/补偿
- 自动发放和失败重试都必须按奖励唯一键与奖励记录ID幂等处理。
- 同一奖励记录不允许出现重复发放；人工终结后不再自动重试。
- CP物料替换必须与奖励发放结果保持一致：新物料发放失败时不得先回收旧物料；自动佩戴切换失败时，不回滚新物料发放结果，进入佩戴状态补偿。
