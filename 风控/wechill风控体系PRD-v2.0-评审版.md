...
>
> **本评审版整合上述三份历史文档，统一对外口径、补齐边界场景、堵住可被 diss 的逻辑漏洞，目标是一次性通过评审。**
...
| 角色 | 关注点 | 对应章节 |
|---|---|---|
| 业务/运营 | 防小号、防套利、防绕过、保收益 | 4 / 5 / 6 / 7 / 8 / 9 |
| 法务/合规 | 个人信息保护、未成年人、自动化决策、申诉权 | 2.3 / 11 / 18 |
| 安全/反欺诈 | 设备指纹强度、IP 风险、模拟器对抗 | 3 / 4 / 12 |
| 财务/结算 | 收益冻结、扣回、退款、拒付、税务 | 10 / 13 |
| 客服/申诉 | 误伤、申诉入口、SLA、客服权限 | 17 / 18 |
| 技术/架构 | 服务可用性、降级、一致性 | 19 / 20 |
| 数据/BI | 看板、误伤率、复犯率、灰度 | 21 / 22 |
| 测试/QA | 验收用例、边界、回归 | 23 |
...
| 制裁国家 | 隐藏国家标识、不支持手机号注册（伊朗、叙利亚、古巴、委内瑞拉、克里米亚、北朝鲜） | 注册前置拦截 |
...
以下截图来自《风控系统需求文档 v1.0》，作为视觉交互留存。**最终实现文案以本 PRD 第 16 章统一文案为准，截图与正文文案不一致时以正文为准。**
...
| 危险分 | < 60 | 危险分 ≥ 60 允许完成注册，登录时拦截 | （注册成功，登录时提示）网络错误 |
...
**设备白名单**：运营可标记公共设备（网吧、门店演示机）为白名单设备，不参与传染。
...
| 设备 200 美金限制 | Toast | — | 当前设备充值金额已达安全限制，暂不支持继续充值。 | — |
| 设备 1000 美金限制 | Toast | — | 当前设备充值金额已达高风险限制，暂不支持继续充值。 | — |
| 设备全面限制充值 | Toast | — | 当前设备存在充值风险，暂不支持充值。 | — |
...
| 危险分 ≥ 60（登录拦截） | 弹窗 | 网络错误 | 网络异常，请检查网络后重试。 | 确定 |
...
Response:
{
  ...
  "reject_message": "当前设备充值金额已达高风险限制，暂不支持继续充值"
}
```
...
|---|---|
| 主风控库故障 | 切换备库，事件回放 |
| 设备指纹库故障 | 降级使用客户端 device_id（仅 30 分钟） |
| IP 信誉库故障 | 降级使用静态规则 |
...
| 模拟器 / 云手机 / Root 设备 | 进入审核，**默认不允许充值**（高于 200 档拦截，0-200 段可放行用于普通用户体验）|
...
- 风控判定服务超时（>2s）→ **fail-close 不创建订单**，前台提示「网络拥塞，请稍后再试」。
...
| AC-DRC-13 | 风控判定服务超时 >2s | 充值订单不创建，前台显示「网络拥塞」，告警写入 |
...
> 4. 客户端文案统一「您当前的设备」（35.6）法务是否同意去掉「请更换网络」？已在 v2.0 第 21.1 节决策为去掉。
