...
同时，Discover Games 页的 `Private room` 作为私密局快捷入口：用户点击后优先在当前首页上拉起休闲游戏选择半屏，选中游戏后直接进入对应 H5 游戏页面，不先进入语聊房主界面，也不经过房内游戏设置页。
...
| Private room 是快捷入口 | Discover 的 Private room 不落到语聊房页面，先选休闲游戏后直达 H5；底层仍走受控游戏会话和资产校验 |
...
| Discover Private room 快捷入口 | 点击后在当前首页拉起休闲游戏选择半屏，选择游戏后直接进入对应 H5，不进入语聊房页和设置页 |
...
![Discover Games 首页-新版国家筛选](outputs/prd-screenshots/01-discover-games-framed-v20260519.png)
...
| Private room | 点击后不进入语聊房，优先在当前页弹出休闲游戏选择半屏 |
...
| 点击游戏 | 关闭半屏并直接进入对应 Bytesun H5 游戏页 |
| 跳转限制 | 不进入语聊房主界面，不进入房内游戏设置页 |
| 关闭弹窗 | 点击关闭按钮或蒙层，返回 Discover Games 当前状态 |
| 异常状态 | 游戏维护、地区不可用、审核隐藏时在列表内灰态或隐藏；不可用时不进入 H5 |

### 4.3 Discover - Activity 活动页

![Discover Activity 活动页-新版](outputs/prd-screenshots/02-activity-framed-v20260519.png)
...
![More Games 全部游戏弹窗-新版](outputs/prd-screenshots/03-more-games-sheet-framed-v20260519.png)
...
| 语聊房内入口 | 房内游戏半屏面板 |
|---|---|
| ![语聊房内入口-新版](outputs/prd-screenshots/04-voice-room-framed-v20260519.png) | ![房内游戏半屏面板-新版](outputs/prd-screenshots/05-room-game-sheet-framed-v20260519.png) |
...
| 与 Private room 区别 | 房内面板保留游戏活动和动态表情；Discover Private room 弹窗仅保留休闲游戏 |
| 无游戏状态 | 点击游戏进入设置页 |
...
![全屏游戏主界面-新版语聊房底部 icon](outputs/prd-screenshots/07-game-main-with-room-icons-framed-v20260519.png)
...
### 4.9 游戏币不足与 Game coins 兑换页

![Game coins 兑换页-新版](outputs/prd-screenshots/08-game-coin-exchange-framed-v20260519.png)
...
flowchart TD
  A["入口"] --> B{"入口来源"}
  B -->|"Discover Private room"| C["当前首页展示休闲游戏选择半屏"]
  C --> D["选择游戏"]
  D --> E["直接进入对应 Bytesun H5"]
  B -->|"房内 Games / 推荐游戏 / More Games"| F["选择游戏"]
  F --> G["打开游戏设置页"]
  G --> H{"用户身份与权限"}
  H -->|"房主 / 可开局管理员"| I["配置人数、门票、模式"]
  H -->|"普通用户"| J["Join / Watch"]
  I --> K["创建 RoomGameSession"]
  J --> K
  K --> L["加载 Bytesun H5"]
  E --> L
  L --> M["gameLoaded"]
  M --> N["waiting / ready 或 private_quick playing"]
  N --> O{"Start / Join / H5 扣费校验"}
  O -->|"游戏币不足"| P["Game coins 兑换页"]
  P --> O
  O -->|"充足"| Q["进入 playing"]
  Q --> R["用户可最小化 / 送礼 / 聊天 / 观战"]
  R --> S["游戏结束 type=14"]
...
### 6.3 Discover Private room 快速开局
...
flowchart TD
  ...
  C --> D{"用户选择游戏"}
  D -->|"游戏可用"| E["关闭半屏"]
  E --> F["创建 private_quick 游戏上下文"]
  F --> G["直接加载对应 Bytesun H5"]
  G --> H["gameLoaded"]
  H --> I["用户在 H5 内开始游戏或进入游戏大厅"]
  D -->|"维护 / 下架 / 地区不可用"| J["灰态或 Toast 拦截"]
  I --> K["最小化时返回 Discover Games"]
```
...
flowchart TD
  ...
  B --> C{"来源页面"}
  C -->|"房内游戏"| D["回到语聊房主界面"]
  C -->|"Discover Private room"| E["回到 Discover Games 首页"]
  D --> F["房间展示完整原生麦位"]
  D --> G["展示游戏浮窗或当前游戏入口"]
  E --> G
  G --> H["入口只展示游戏名/状态/人数/Open，不展示麦位和余额"]
  H --> I["用户点击 Open"]
  I --> J["校验用户仍在房间或私密游戏上下文有效"]
  J --> K["查询 RoomGameSession"]
  K --> L["查询 RoomGamePlayer"]
  L --> M["查询 RoomRoleState / RoomMicState / RTCState / WalletBalance"]
  M --> N["恢复或重建 H5 容器"]
  N --> O["向 H5 发送展开状态并刷新画面"]
```
...
| 入口配置 | Discover 推荐位、Private room 快捷入口、More Games、房内入口、国家筛选、活动入口 |
...
| source | room_panel / discover_recommend / more_games / private_quick |
...
| join_source | room_panel / discover / private_room_quick / invite / restore |
...
| return_context | 原游戏 session / room / discover_private |
...
| private_room_game_select | Private room 半屏内选择游戏 |
...
| Private room 直达加载成功率 | `game_loaded / private_room_game_select` |
...
- 用户在 Private room 半屏中选择游戏后，直接进入对应 H5 游戏页，不进入语聊房主界面，不进入游戏设置页。
